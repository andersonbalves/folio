...
from folio_mcp.tools.get_document import get_document
...
async def test_get_document_not_found(mock_conn_ctx, mock_cursor):
    ...
    with patch("folio_mcp.tools.get_document.conn", mock_conn_ctx):
        result = await get_document("nonexistent/doc.md")
